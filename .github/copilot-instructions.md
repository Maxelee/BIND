...
Matured 2D analyses on top of a trained two-head model. The `scatter/` package
(BIND scatter paper) is the core: `obs_common.py` defines the 11 group-scale
observables + geometry (`PATCH_PIX=128`, must match `fd_jacobian_cv.py`);
`measure_scatter.py` / `calibration_cv.py` / `scatter_decomposition.py` /
`sensitivity.py` / `residual_pipeline.py` are the engine. `project1`–`project7`
are self-contained figure drivers (`paper_figures/projN_*`). Conventions: heavy
intermediate products live in `analysis_physics_cache/` (gitignored) and are
reused, not re-sampled; some notebooks are generated by `build_*.py` / `gen_*.py`
(edit the notebook after, or regenerate to clear outputs).
