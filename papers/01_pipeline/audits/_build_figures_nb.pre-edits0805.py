...
#       runs/dmo/run_0000/Cl_kappa.npz (S(ell) denominator), PLUS
...
NPIX = BK.shape[-2]*BK.shape[-1]
del BK, TK, bk, tk; gc.collect()   # nothing below needs the full kappa cubes
...
print("(peaks/minima/pdf point errors = Poisson sqrt(mean count)/count; MF point "
      "errors = seed-paired per-realization DIFFERENCE scatter/sqrt(50) -- Poisson "
...
axis-Nyquist $\ell=36864$), shown for completeness rather than as a closure claim. The SZ
panels shade the cosmic-variance-only floor ($f_{\rm sky}=0.44$, mode counting alone): a
formal shape-noise forecast has no meaning for a Compton-$y$ field, so this is an honest
*lower bound* on achievable precision, not a survey forecast.
...
def panel_data(s):
    ...
    # silently rescaled the axis by a further ~1/sigma0 (a few tens x).
