...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig01_transfer_ensemble.pdf}
...
for 99 SB35 Sobol realizations at five source planes (labeled $\zs$ in each
panel), computed against a single fixed, paired DMO skeleton and colored by
each run's group gas fraction $\fgas$; the near-black solid/dashed lines
mark the ensemble median and 95\% envelope. The ensemble brackets both
suppression and enhancement relative to the paired DMO trace, and the
spread narrows toward higher $\zs$ as the low-redshift baryonic signal is
diluted by feedback-free structure along the line of sight. The $x$-axis is
capped at $\ell_{\rm max,trust}=1.5\times10^4$, above which CIC aliasing
dominates the run-to-run scatter. The $\zs=0.5$ panel's $y$-axis is fixed
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig02_transfer_tomography.pdf}
...
structure (colors as in the left panel).}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig03_bias_scatter.pdf}
...
(99 runs), colored by group $\fgas$, compared to the $1\sigma\,S_8$
LSST-Y10 uncertainty reference bar (vertical only; the corresponding
$\sigma_{\Omega_{\rm m}}$ intermediate is not part of the cached Fisher
output). The bias traces a tight, nearly one-dimensional locus.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig04_template_ladder.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig05_beyond2pt_manifold.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig06_bias_sensitivity.pdf}
...
\begin{figure}[t]
\centering
\todo{regenerate: no cached final-product array -- \texttt{lightcone\_selfcal.py}
computes the joint $(S_8,\Omega_{\rm m},A_{\rm bary})$ Fisher forecast fresh
at plot time via \texttt{pyccl} and never calls \texttt{np.savez}; re-running
the Fisher/pyccl calculation is out of scope for this figure-integration pass
(see \texttt{fig\_scripts/UNREGENERATED.md})}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig10_bias_tomography_superseded.pdf}
...
\begin{figure}[t]
\centering
\todo{regenerate: no cached final-product array -- the BCM/van~Daalen/BCemu
analytic-model spans in \texttt{lightcone\_model\_comparison.py} are computed
fresh at plot time via \texttt{pyccl} (\texttt{BaryonsSchneider15},
\texttt{BaryonsvanDaalen19}) and the external \texttt{BCemu} package, with no
\texttt{np.savez} call; only the BIND envelope half is cached
(\texttt{transfer\_Sell.npz}), which is insufficient to reproduce the
three-way comparison; re-running the analytic engines is out of scope for
this figure-integration pass (see \texttt{fig\_scripts/UNREGENERATED.md})}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig09_rescale_validation.pdf}
...
% ============================================================
% Figure verification log (2026-07-16, FIGURE FIX pass)
% ============================================================
% Scope: two adversarial figure verifiers reviewed all 10 figures (8
% regenerated fig_scripts/*.py + 2 kept placeholders fig07/fig08). Verifier 1
% found one MEDIUM finding; verifier 2 (independent pass, data-provenance +
% caption cross-check) found none.
% Applied (MEDIUM):
%  - FIGURE_STYLE.md rule 7 violation: several fig_scripts manually shrank
%    fonts below the paper_style.py rcParams defaults (font.size=8,
%    legend.fontsize=7, tick labels=7) instead of resizing the figure.
%    Worst offender was fig05_beyond2pt_manifold.py's 10x10 subspace-
%    alignment matrix (panel b): legend fontsize=5.2, tick-label
%    fontsize=5.5, in-cell value annotations fontsize=4.3. Fixed by
%    re-laying out fig05 as a taller, stacked (not side-by-side) figure
%    (TWO_COL width x 8.4in) so panel (b) gets close to the full page width;
%    all fontsize= overrides removed, everything now renders at the style's
%    default sizes. (This also required moving panel (b)'s "(b)" tag from
%    the standard panel_label() corner to just above the axes, since every
%    corner cell of the matrix holds real data -- e.g. the diagonal (0,0)
%    cell is 1.00 -- and the default corner placement collided with it.)
%  - Same rule 7 violation, smaller instances: fig04_template_ladder.py's
%    in-axes "solid/dashed" legend note (fontsize=6), fig06_bias_sensitivity
%    .py's 12-parameter y-tick labels (fontsize=6), and
%    fig09_rescale_validation.py's per-panel (s, z_*) annotation
%    (fontsize=5.5) and top-left legend (fontsize=6). All four fontsize=
%    overrides removed; at default sizes every panel remains legible with
%    no clipping or overlap at native print resolution (re-checked via
%    cropped zooms of figs_preview/*.png, ~193-200 dpi at TWO_COL/\textwidth
%    scale, matching the verifier's own methodology).
% Data/content: unchanged. No cached array, curve, point, or matrix value was
% altered by this pass -- only matplotlib layout/font parameters. Figures
% fig01, fig02, fig03, fig10 (not flagged) and the fig07/fig08 kept
% placeholders were not touched. Recompiled clean (pdflatex x2 + bibtex +
% pdflatex): no undefined references/citations, no LaTeX errors.
% ============================================================
