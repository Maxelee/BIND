...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig01_latent_axes_zones.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{figs/fig02_latent_physical.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figs/fig02b_latent_plane.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{figs/fig03_wl_ksz_geometry.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\textwidth]{figs/fig04_multistat_align.png}
...
nearly universal ($\geq0.93$); latent 2 (orange) ranges from $0.20$
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig05_spearman_response.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figs/fig06_npe_corner.png}
...
for this posterior (\S\ref{sec:methods-sbi}; WORKLOG 2026-06-29 §3), but we
note that Figure~\ref{fig:sbc}a, despite its in-panel title, does not
actually plot the promised \texttt{emcee} overlay (see the figure caption's
caveat); we therefore do not cite that panel as visual evidence of NPE/emcee
agreement, only the SBC coverage result above, which is not affected by
this issue.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig08_sbc_crosschecks.png}
\caption{Cross-checks on the NPE posterior. (a) NPE posterior marginals
(unit-prior units) for three representative parameters; the in-panel title
anticipates an overlaid explicit-likelihood \texttt{emcee} comparison
(``NPE solid vs.\ emcee dashed''), but as extracted from the source notebook
the panel contains only the three solid NPE curves — no dashed \texttt{emcee}
curves are actually rendered. We flag this discrepancy rather than silently
correct or omit it; \todo{re-generate this panel with the emcee marginals
actually overlaid, or confirm none were computed for these three
parameters.} (b) Simulation-based calibration rank
...
$=0.70$ (red dashed line). % src: f3e\_crosschecks.png; panel (a) checked
% directly against the rendered image, no dashed lines present despite the
% title and legend implying an emcee overlay
...
posteriors above; panel (a)'s emcee cross-check is described in
\S\ref{sec:methods-sbi} but is not visually substantiated by this figure.}
\label{fig:sbc}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig09_progressive_stacking.png}
...
per-parameter, for the ten best-constrained parameters. Two of the
$y$-axis tick labels in this panel are truncated to the identical string
``WindEnergyReductio,'' which the SB35 parameter table shows corresponds to
two (of three) distinct parameters —
\emph{WindEnergyReductionFactor}, \emph{WindEnergyReductionMetallicity}, and
\emph{WindEnergyReductionExponent} all truncate identically; we cannot
determine from the extracted image alone which two of the three are shown,
so we flag this as a labeling ambiguity in the source figure rather than
guess. \todo{Re-generate this panel with untruncated parameter labels.} %
% src: f3d\_progressive.png; src/bind/assets/SB35\_param\_minmax.csv lines
% 19--21 (three \emph{WindEnergyReduction*} parameters)
...
``redistribution'' features ($r\approx0.47$, blue). Right: per-halo Born
...
\caption{$\bind{}$ shear$\times y$ predictions (red; $16$–$84\%$ Sobol
...
\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figs/fig13_shmr_scatter_budget.png}
...
parameters (``feedback,'' orange) versus the halo-to-halo (non-feedback)
residual (blue), with the twobound one-at-a-time marginal scatter overlaid
as a dashed reference line. % src:
