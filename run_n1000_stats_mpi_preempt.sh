#!/bin/bash
#SBATCH --job-name=bind_n1000_stmpre
#SBATCH --output=/mnt/home/mlee1/ceph/logs/bind_n1000_stmpre_%A_%a.out
#SBATCH --error=/mnt/home/mlee1/ceph/logs/bind_n1000_stmpre_%A_%a.err
#SBATCH --partition=preempt
#SBATCH --qos=preempt
#SBATCH --constraint="icelake|cascadelake|skylake"
#SBATCH --nodes=1
#SBATCH --ntasks=48
...
# ── N1000 stats, MPI over realizations — PREEMPT QOS variant ─────────────────
# Same work as run_n1000_stats_mpi.sh, submitted against the preempt QOS so it
# does not queue behind the trace lanes holding the cca node cap.  A run takes
# ~5 min, so preemption barely matters here (the old 3 h serial job could never
# survive a preempt window); --requeue plus the convention-aware guard means a
# killed task simply redoes its run, and a half-written npz is detected and
# recomputed rather than trusted.  48 ranks fits the 48-core pools too, so it
# can land on icelake OR cascadelake/skylake — whichever frees first.
#
...
#   N_REAL=32 RUN_DIR_OVERRIDE=<scratch> sbatch --array=2 run_n1000_stats_mpi_preempt.sh
