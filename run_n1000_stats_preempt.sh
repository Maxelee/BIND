#!/bin/bash
#SBATCH --job-name=bind_n1000_st_pre
#SBATCH --output=/mnt/home/mlee1/ceph/logs/bind_n1000_st_pre_%A_%a.out
#SBATCH --error=/mnt/home/mlee1/ceph/logs/bind_n1000_st_pre_%A_%a.err
...
#SBATCH --array=0-185%40
#SBATCH --mail-user=mel2260@columbia.edu
#SBATCH --mail-type=FAIL

# ── N1000 stats on the PREEMPT QOS (the cca QOS is at its 500-task submit cap) ─
# Identical work to run_n1000_stats.sh, submitted against the preempt QOS, which
# carries its own separate MaxSubmitPU=500 — so this does not compete for the
# cca submit budget the trace lanes are using.
#
# Why this starts quickly even behind queued traces: a stats task is 1 node for
# a few hours, so Slurm's backfill drops it into gaps that a 3-4 node trace job
# cannot fill.  You cannot raise a job's priority above your own older queued
# array tasks (negative --nice needs operator rights), but you rarely need to:
# small jobs backfill past big ones.  Preemption is safe here — --requeue plus
# the load-verify skip check (a truncated npz from a killed savez is detected
# and recomputed, never mistaken for a finished product).
#
# Array range 0-185 fits the free preempt submit slots (500 cap - the trace
# array's ~314).  Tasks whose run is not yet traced exit in seconds, so just
# resubmit (with other --array ranges, e.g. 186-318) as runs land and slots free.
#
# Env overrides: OUT_ROOT, SMOOTHING, NGAL, SIGMA_E, STATS_EXTRA, FORCE.
...
: "${SLURM_ARRAY_TASK_ID:?run_n1000_stats_preempt.sh must run as a SLURM array task}"
...
echo "=== n1000 stats (preempt) | task $IDX -> $OUT_RUN | $(date) ==="
