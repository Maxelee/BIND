#!/bin/bash
#SBATCH --job-name=fm_testsuite_no_cosmo
#SBATCH --output=/mnt/home/mlee1/ceph/logs/fm_testsuite_no_cosmo_%A_%a.out
#SBATCH --error=/mnt/home/mlee1/ceph/logs/fm_testsuite_no_cosmo_%A_%a.err
...
RUN_DIR=${RUN_DIR:-/mnt/home/mlee1/ceph/fm_runs/fm_two_head_no_cosmo}
MODEL_NAME=${MODEL_NAME:-fm_two_head_no_cosmo}
...
# ── Manifest generation (task 0 builds it if missing; others wait) ────────────
# The DM projections, halo catalogs and cutouts are shared with prior runs and
# are NOT regenerated (no --regenerate_all).  Only generated_halos_*.npz and
# composite_*.npz files for this model_name will be created from scratch.
MANIFEST_LOCK="$MANIFEST_DIR/.manifest.lock"

if [[ "$CHUNK_ID" == "0" ]]; then
    if [[ -f "$TEST_MANIFEST" ]]; then
        echo "=== [chunk 0] Manifest already exists at $TEST_MANIFEST — reusing ==="
        touch "$MANIFEST_LOCK"
    else
...
# ── Build extra flags ─────────────────────────────────────────────────────────
# No --regenerate_all: reuse existing DM projections, halo catalogs, and cutouts.
# Only generated_halos and composites for this model_name are written fresh.
...
[[ "${REPASTE:-0}" == "1" ]]       && EXTRA_FLAGS+=(--repaste)

echo "=== [chunk $CHUNK_ID/$N_CHUNKS] Running generation (no_cosmo model) ==="
