#!/bin/bash
#SBATCH --job-name=bind_tb_lc
#SBATCH --output=/mnt/home/mlee1/ceph/logs/bind_tb_lc_%A_%a.out
#SBATCH --error=/mnt/home/mlee1/ceph/logs/bind_tb_lc_%A_%a.err
...
# ── BIND twobound lightcone: full lux ray-trace per parameter run ──────────────
# One SLURM array task = one twobound run (run_<idx>).  Mirrors the FIDUCIAL
# pipeline (run_lightcone_tau.sh) exactly, but builds every stage from the saved
# per-halo patches, since the twobound runs only store generated_patches/
# thermo_patches (no pasted composites, no lensplanes, no rt_output):
#
...
#   4. collect -> {kappa,y,tau}_maps.npz   (overwrites the older kappa/y maps with
#      a self-consistent kappa+y+tau set on one geometry).
...
#
# Env overrides: TB, FID, N_REAL, LP_GRID, RT_GRID, FOV_DEG, RT_SEED, COMPUTE_TSZ,
#                LUX_BIN, WORK_ROOT, PAINT_PARALLEL, FORCE.

set -euo pipefail

# ── shared paths / config ─────────────────────────────────────────────────────
TB=${TB:-/mnt/home/mlee1/ceph/bind_science/runs/twobound}
