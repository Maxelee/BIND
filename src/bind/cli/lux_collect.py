...
Reads the per-realization ``run<NNN>/kappa{plane}.dat`` (and ``y{plane}.dat`` if
present) under a lux ``RT_output_dir`` and writes compact ``kappa_maps.npz`` /
``y_maps.npz`` (shape ``(n_real, n_src, npix, npix)``) into ``--output_dir`` for
the stats stage.  Optionally deletes the raw ``.dat`` afterwards (the npz is the
kept data product).
...
from bind.inference.lux_io import (
    load_kappa_realizations, load_y_realizations, PLANE_TO_ZS,
)
...
def main() -> None:
    ...
    kappa, zs = load_kappa_realizations(args.rt_root, planes=planes)
    ...
    try:
        y, _ = load_y_realizations(args.rt_root, planes=planes)
