...
PER_HALO_KEYS = [
    ...
    "dq_dm",
    # ── THERMO additions (fm_thermo only): tSZ + X-ray aperture observables ──
    # computed by thermo_observables() from the 7-channel physical map. Validated
    # against the Sobol cube (Y200/SX corr=1.0000); see tools/fd_thermo_observables.py.
    "Y200", "SX",
]
...
def thermo_observables(phys_7HW, r200_pix):
    """tSZ + X-ray aperture observables from a 7-channel physical map
    [DM,Gas,Stars,Y,T,S,P]. Aperture matches observables_from_phys (clamped R200c).
    SX proxy = sum(Gas^2 sqrt(T)) (n_e^2 sqrt(T)). Validated vs the Sobol cube
    (Y200/SX corr=1.0000); see tools/fd_thermo_observables.py.
    """
    ...
    return {"Y200": float(np.maximum(Y, 0.0)[mask].sum()),
            "SX":   float(SXmap[mask].sum())}
...
def run_compute(args):
    ...
    # ── fiducial observables (param-independent) — needed to turn the raw
    #    dO/dtheta into a log-Jacobian dO/O for the Fisher chain ───────────────
    F_fid = {k: np.full(N_USE, np.nan, dtype=np.float32) for k in PER_HALO_KEYS}
    with torch.no_grad():
        for start in range(0, N_USE, args.batch_size):
            ...
            p_b = p_base.unsqueeze(0).expand(B, -1).contiguous()
            xf = _sample_fixed_noise(model_fm, cb, lb, p_b, z_use[start:stop], args.n_steps)
            phys_f = _denormalize_to_physical(xf.cpu().numpy(), norm_stats)
            for b, h_local in enumerate(range(start, stop)):
                h = idx_use[h_local]
                of = observables_from_phys(phys_f[b], r200_pix[h], q_DMO[h])
                of.update(thermo_observables(phys_f[b], r200_pix[h]))
                for k in PER_HALO_KEYS:
                    F_fid[k][h_local] = of[k]
    print("[compute] fiducial observables done")

    # fiducial-only mode: save just the fiducial observables (for retro-fitting an
    # earlier FD run that predates Ffid) and exit before the (expensive) param loop.
    if getattr(args, "fiducial_only", False):
        out_path = Path(args.output); out_path.parent.mkdir(parents=True, exist_ok=True)
        save = {f"Ffid_{k}": F_fid[k] for k in PER_HALO_KEYS}
        save["idx_use"] = idx_use; save["sim_id_use"] = cv["sim_id"][idx_use]
        save["masses_use"] = masses_use
        np.savez_compressed(out_path, **save)
        print(f"[compute] fiducial-only -> wrote {out_path}")
        return
        ...
        with torch.no_grad():
            for start in range(0, N_USE, args.batch_size):
                ...
                for b, h_local in enumerate(range(start, stop)):
                    ...
                    obs_p.update(thermo_observables(phys_p[b], r200_pix[h]))   # Y200, SX
                    obs_m.update(thermo_observables(phys_m[b], r200_pix[h]))
                    for k in PER_HALO_KEYS:
    ...
    save.update({f"Ffid_{k}": F_fid[k] for k in PER_HALO_KEYS})   # fiducial obs (for log-Jacobian)
...
def run_merge(args):
    ...
    # fiducial observables are param-independent -> take from the first shard
    Ffid_full  = {k: first[f"Ffid_{k}"] for k in PER_HALO_KEYS if f"Ffid_{k}" in first.files}
    ...
    save.update({f"Ffid_{k}": v for k, v in Ffid_full.items()})
...
def main():
    ...
    ap.add_argument("--fiducial_only", action="store_true",
                    help="Only compute+save the fiducial observables Ffid_* (one pass), "
                         "to retro-fit an FD run that predates fiducial saving.")
